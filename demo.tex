...
Table~\ref{tab:nekrs} compares scalability of NekRS (GPU) and NekRS (CPU) on ORNL’s Summit, which is equipped 
with 42 CPU cores and 6 Nvidia V100s per node. For the same node count, the GPU-accelerated variant of NekRS 9 to 14
times faster, depending on the case. NekRS realizes 80% efficiency at 2 million points per GPU.
...
\begin{table}
 ...
 % \multirow{2}{*}{ } &
  ...
  \hline
  %\cline{2-5}
    nodes  &  $E$ per GPU & time per step (sec) & efficiency & E per MPI rank& time per step (sec) & efficiency \\
  \hline
